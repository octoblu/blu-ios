...
class AppDelegate: UIResponder, UIApplicationDelegate, WCSessionDelegate {
  ...
  let provider = Provider(
    clientID:     "5a43d28a-841e-47c8-b039-1056e2451e6a",
    authorizeURL: "https://oauth.octoblu.com/authorize",
    redirectURL:  "blu://blu.octoblu.com/callback"
  )
  ...
  func session(session: WCSession, didReceiveMessage message: [String : AnyObject], replyHandler: ([String : AnyObject]) -> Void) {
    ...
    case "refreshFlows":
      self.refreshFlows({ (triggers) -> Void in
        ...
        if triggers != nil {
          for (_, element) in triggers!.enumerate() {
            flows.append(element.asDictionary())
          }
        }
  ...
  func refreshFlows(onSuccess : ((triggers : [Trigger]?) -> Void)?) {
    ...
    let bearerToken = settings.getTokenForProvider(self.provider)
    
    if (bearerToken == nil) {
      if self.window != nil {
        
        dispatch_async(dispatch_get_main_queue()) { () -> Void in
          self.provider.state = NSUUID().UUIDString.lowercaseString
          self.provider.authorize { (result: Result<Token, Error>) -> Void in
            switch result {
      ...
      self.octoblu = Octoblu(bearer: bearerToken!.accessToken.stringByRemovingPercentEncoding!)
    }
